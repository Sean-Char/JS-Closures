...
function findPotentialFriends(existingFriends) {
  return function notAfriendYet(person) {
    if (existingFriends.indexOf(person) < 0) {
     console.log(person, "is not a friend")
      return true
    }
    console.log(person, "is a friend")
      return false
  }
}
...
var potentialSecondLevelFriends = secondLevelFriends.filter(function (name, idx){
  return isNotAFriend(name);
});
var allPotentialFriends = allUsers.filter(function (name, idx){
  return isNotAFriend(name);
})
